        ...
        viewModelScope.launch(Dispatchers.IO) {
            if (inserting) {
                ...
                if (currentTiming.duration >= ignoreLessThan) {
                    ...
                    Log.d(TAG,"saveTiming: deleting timing with duration ${currentTiming.duration}")
                    getApplication<Application>().contentResolver.delete(TimingsContract.buildUriFromId(currentTiming.id),values,null)
                }
